...
namespace PainelWeb.Data.MYSQL
{
    public class MYSQLEventos : IEventoDb
    {
        private readonly string _conexao;

        public MYSQLEventos(ConexaoBanco conexaoBanco)
        {
        ...
        {
            var sql = $@"UPDATE Eventos SET HoraFim = now() WHERE HoraFim IS NULL AND Local = @Local";

            using (var banco = new MySqlConnection(_conexao))
            {
...
HoraFim IS NULL";
        ...
        {
            var sqlUpdate = $@"UPDATE Eventos SET HoraFim = now() WHERE HoraFim IS NULL AND Local = @Local AND Usuario = @Usuario OR Usuario is NULL ";

            var sqlInsert = $@"INSERT INTO Eventos (IdElise, HoraInicio, Dispositivo, Local, TextoEvento, Usuario, HoraInsert, Tipo)
                               VALUES
                               (1, now(), 1, @Local, @TextoEvento, @Usuario, now(), @Tipo)";
        ...
        {
            var sqlUpdate = $@"UPDATE Eventos SET HoraFim = now() WHERE HoraFim IS NULL AND Local = @Local";

            var sqlInsert = $@"INSERT INTO Eventos (IdElise, HoraInicio, Dispositivo, Local, TextoEvento, HoraInsert, Tipo)
                               VALUES
                               (1, now(), 1, @Local, @TextoEvento, now(), @Tipo)";
